...
public class ArcaneOrbAttack extends BossAttackBase {
    public ArcaneOrbAttack(ArmorStandBoss boss) {
        super(boss);
    }
    ...
    public void execute(BossInstance instance) {
        ...
        new BukkitRunnable() {
            ...
            @Override
            public void run() {
                if (stand.isDead() || !stand.isValid() || t > 120) { cancel(); return; }
                if (t < 25) {
                    ...
                    if (t % 3 == 0) world.spawnParticle(Particle.END_ROD, center.clone().add(0, 1.5, 0), 3, 0.3, 0.3, 0.3, 0.01);
                } else {
                    if (target.isOnline()) {
                        ...
                        if (p.getLocation().distanceSquared(pos) < 6) {
                            ...
                            for (Player near : boss.getValidPlayers(world)) {
                                if (near.getLocation().distanceSquared(pos) < 25) {
                                    ...
                                    if (away.lengthSquared() > 0) near.setVelocity(away.normalize().multiply(0.8).setY(0.4));
                                }
                            }
