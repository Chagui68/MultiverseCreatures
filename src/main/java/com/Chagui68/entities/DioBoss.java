...
public class DioBoss implements Listener {
            ...
            public void run() {
                ...
                if (!instance.isStandPunchingActive()) {
                    if (stand.getLocation().getChunk().isLoaded()) {
                        double offsetZ = plugin.getConfig().getDouble("dio-boss.stand-offset-z", -1.0);
                        double offsetY = plugin.getConfig().getDouble("dio-boss.stand-offset-y", 2.0);
                        Location behind = zombie.getLocation().clone()
                                .add(zombie.getLocation().getDirection().multiply(-1).setY(0).normalize().multiply(Math.abs(offsetZ)))
                                .add(0, offsetY, 0);
                ...
                if (zombie.getTarget() instanceof Player target && target.isOnline()) {
                    ...
                    long cooldownMs = (long) (plugin.getConfig().getLong("dio-boss.cooldown-ms", 120000));

                    double freezeRadius = plugin.getConfig().getDouble("dio-boss.freeze-radius", 50.0);
                    double freezeDamageRadius = plugin.getConfig().getDouble("dio-boss.freeze-damage-radius", 30.0);
                    double freezeDamage = plugin.getConfig().getDouble("dio-boss.freeze-damage", 10.0);
                    int freezeDurationTicks = plugin.getConfig().getInt("dio-boss.freeze-duration-ticks", 100);
                    double teleportInnerRadius = plugin.getConfig().getDouble("dio-boss.teleport-inner-radius", 25.0);
    ...
    private void performTheWorldFreezing(Zombie zombie, List<Player> nearby, double freezeRadius, double freezeDamageRadius, double damage, int durationTicks) {
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (p.getWorld().equals(center.getWorld()) && p.getLocation().distanceSquared(center) <= freezeRadius * freezeRadius) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (phase == 0) {
                    ...
                    if (phaseTicks == 0) {
                        for (Map.Entry<Player, List<ItemDisplay>> entry : playerSwords.entrySet()) {
    ...
    private void performTheWorldTeleport(Zombie zombie, Player target) {
        ...
        Location teleportPos = target.getLocation().clone().add(0, 0, -2);
        zombie.teleport(teleportPos);

        int darknessDuration = plugin.getConfig().getInt("dio-boss.teleport-darkness-duration", 100);
        int slownessDuration = plugin.getConfig().getInt("dio-boss.teleport-slowness-duration", 100);

        target.addPotionEffect(new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.DARKNESS, darknessDuration, 0, false, true));
        target.addPotionEffect(new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.SLOWNESS, slownessDuration, 0, false, true));
    ...
    private List<Player> getNearbyPlayers(Location center, double radius) {
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (p.getWorld().equals(center.getWorld()) && p.getLocation().distanceSquared(center) <= radiusSq) {
                result.add(p);
            }
        }
        return result;
